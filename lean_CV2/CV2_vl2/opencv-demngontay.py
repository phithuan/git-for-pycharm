...
FolderPath="Fingers"
lst=os.listdir(FolderPath)#Phương thức listdir() của Python trả về một danh sách chứa tên của các mục trong thư mục được cung cấp bởi đường dẫn
#print(lst)
lst_2=[]  # khai báo list chứa các mảng giá trị của các hình ảnh/
for i in lst:
    #print(i)
    image=cv2.imread(f"{FolderPath}/{i}")  # Fingers/1.jpg , Fingers/2.jpg ...#Một chuỗi ký tự được định dạng hoặc chuỗi f là một chuỗi ký tự,Các chuỗi này có thể chứa các trường thay thế, là các biểu thức được phân tách bằng dấu ngoặc nhọn {}.
